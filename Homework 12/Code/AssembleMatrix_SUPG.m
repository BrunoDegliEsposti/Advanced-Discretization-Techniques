...
function [A_SUPG] = AssembleMatrix_SUPG(coord, elemNodeTable, ...
        deltaT_SUPG, coeff_c, coeff_r)
    %ASSEMBLEMATRIXSUPG Assemble SUPG matrix using Lagrange P1 elements
    n_vertices = size(coord, 1);
    n_elem = size(elemNodeTable, 1);
    
    A_SUPG = sparse(n_vertices, n_vertices);
    ...
    for i = 1:n_elem
        ...
        % evaluate the coefficients c(x,y) and r(x,y)
        ...
        A_SUPG_local = (1/3) * el_area * DN' * Dinv * c * (...
            c'*Dinv'*DN + r*N);
        
        % contribution added to the global matrix
        A_SUPG(v_elem, v_elem) = A_SUPG(v_elem, v_elem) + ...
            deltaT_SUPG(i)*A_SUPG_local;
    end
end
