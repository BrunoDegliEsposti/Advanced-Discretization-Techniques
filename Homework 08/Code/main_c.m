% PROGRAMMING EXERCISE 7: RAVIART-THOMAS ELEMENTS POINT C)
% Bruno Degli Esposti, Xingyu Xu
% 10/12/19 - 17/12/19
% Code tested in MATLAB only

% Data: right-hand side f and Dirichlet boundary values
f = @(x,y) 0;
u_D = @(x,y) 4.*x.*(1-x).*(abs(y+1)<1e-8);

% Load mesh from file
mesh = RTMesh('meshes/lshaped-3.msh');
...
% f = 0 everywhere, nothing to do

% Incorporate Dirichlet conditions
for j = 1:size(mesh.boundaryEdges, 1)
    v1 = mesh.boundaryEdges(j,1);
    v2 = mesh.boundaryEdges(j,2);
    e = mesh.nodes2edge(v1,v2);
    l = mesh.calcEdgeLength([v1,v2]);
    m = mesh.calcMidpoint([v1,v2]);
    b(e) = b(e) + l*u_D(m(1),m(2));
end
...
% Plot results
figure(1)
mesh.plotConstantFunction(uh, 'u');

figure(2)
mesh.plotRTFunction(sigmah, 'Du');
