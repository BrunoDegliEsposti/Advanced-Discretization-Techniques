% PROGRAMMING EXERCISE 7: RAVIART-THOMAS ELEMENTS POINT B)
...
for i = 1:5
    % Load mesh from file
    mesh = RTMesh(sprintf('meshes/square-%d.msh',i));
    ...
    for j = 1:mesh.numTriangles
        ...
        x = mesh.calcMidpoint(v);
        area = abs(mesh.calcTriangleArea(v));
        local_err = u(x(1),x(2)) - uh(j);
        errL2(i) = errL2(i) + area*local_err^2;
    end
...
hold on;
loglog(h,h.^2,'k--');
xlabel('Diameter h of the triangulation');
legend('L^2 norm of the error','h^2, as a reference','Location','northwest');
