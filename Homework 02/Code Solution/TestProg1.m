%Tests AssembleSystem.m in the case of the stationary elliptic problem
...
for N = 1+[4,8,16,32,64,128]
    ...
    % The drawback of this method is that B loses its symmetry.
    % However, solving the linear system is definitely not the bottleneck
    % in this finite elements code, so it doesn't make any difference.
    for i = 1:n_boundary_edges
        % 1 means Dirichlet boundary condition
        if boundary(i,1) == 1
            ...
            % Second vertex of the edge. Often redundant, but not always.
...
figure(1);
subplot(1,2,1), trisurf(elemNodeTable, coord(:,1), coord(:,2), uh), ...
    shading interp;
subplot(1,2,2), trisurf(elemNodeTable, coord(:,1), coord(:,2), u), ...
    shading interp;
figure(2);
